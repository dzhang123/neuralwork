...
class Network(object):
    ...
    def SGD(self, 
            ...
            mini_batch_size: int, 
            eta: float, 
            test_data: list[tuple[np.ndarray, np.ndarray]] = None) -> None:
        """Train the neural network using mini-batch stochastic gradient descent.
        The "training_data" is a list of tuples "(x, y)" representing the training inputs and the desired outputs.
        The other non-optional parameters are self-explanatory. If "test_data" is provided then the network will 
        be evaluated against the test data afater each epoch, and partial progress printed out. This is useful for 
    ...
    def update_mini_batch(self, mini_batch: list[tuple[np.ndarray, np.ndarray]], eta: float) -> None:
        ...
        single mini batch. The "mini_batch" is a list of tuples "(x, y)", and "eta" is the learning rate."""
    ...
    def backprop(self, x: np.ndarray, y: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
